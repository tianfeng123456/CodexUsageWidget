...
测试先预热 12 秒，再要求 SQLite 连续稳定 5 秒，随后正式观察 65.018 秒。窗口在
...
| CPU 增量 | 0.046875 秒 |
| 单核口径平均 CPU | 0.0721% |
| 16 线程整机口径平均 CPU | 0.0045% |
| 私有内存 | 81,551,360 字节（77.77 MiB） |
| 峰值私有内存 | 84,054,016 字节（80.16 MiB） |
| 工作集 | 172,126,208 字节（164.15 MiB） |
| 正式观察期数据库变化 | 0 |
